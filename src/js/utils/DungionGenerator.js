...
export class DungeonGenerator {
    ...
    generate(dungeonArgs) {
        // If a seed is provided, use it to generate dungeon.
        if (dungeonArgs.seed) {
            seedrandom(dungeonArgs.seed, { global: true });
        }

        const startAt = Date.now();

        const tree = this.createTree(dungeonArgs);
        ...
        return {
            width: dungeonArgs.mapWidth,
            height: dungeonArgs.mapHeight,
            tree,
            // layers: {
            //   tiles,
            //   props,
